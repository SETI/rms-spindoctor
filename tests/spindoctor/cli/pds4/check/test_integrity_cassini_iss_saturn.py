...
finding the change: a file a label names that is not there, a file no label names or two
labels name, a template's marker, an empty element, and a reference to a product of the
bundle, or to a version of one, that the tree does not hold.
"""
...
def test_a_file_its_label_names_that_is_not_beside_it_is_found(
    ...
    supplemental = (bundle / label).with_name(
        (bundle / label).name.replace('_backplanes.lblx', '_supplemental.txt')
    )
    supplemental.unlink()
    expected = Finding(
        label,
        CheckName.INTEGRITY,
        '/Product_Observational/File_Area_Observational_Supplemental/File/file_name',
...
def test_an_empty_element_is_found(plain_bundle: Path, tmp_path: Path) -> None:
    ...
    expected = Finding(
        KERNELS, CheckName.INTEGRITY, KERNELS_TITLE, 'is empty, and does not carry xsi:nil'
    )
    assert expected in _check(bundle)


def test_an_empty_element_carrying_xsi_nil_is_not_found(plain_bundle: Path, tmp_path: Path) -> None:
    """An element holding nothing that carries ``xsi:nil`` is empty on purpose."""
    bundle = copy_bundle(plain_bundle, tmp_path)
    substitute_once(bundle / KERNELS, r'<title>[^<]*</title>', '<title xsi:nil="true"></title>')
    at_title = [
        finding
