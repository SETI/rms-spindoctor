...
def _sim_snapshot_with_moon_a() -> tuple[HermeticObs, np.ndarray]:
    """Build a simulated snapshot containing one masked body, MOON_A.

    Returns:
        The observation and the full-frame boolean MOON_A mask (v 2..3, u 3..5).
    """
    mask = np.zeros(SHAPE_VU, dtype=bool)
    mask[2:4, 3:6] = True
    inventory = {'MOON_A': inventory_entry(u_min=3, u_max=5, v_min=2, v_max=3, body_range=500000.0)}
    snap = make_snapshot(
        shape_vu=SHAPE_VU,
        simulated=True,
        sim_inventory=inventory,
        sim_body_mask_map={'MOON_A': mask},
    )
    return snap, mask
...
def test_simulated_body_mask_matches_sim_mask() -> None:
    ...
    snap, mask = _sim_snapshot_with_moon_a()
    result = create_body_backplanes(snap, _bodies_config().as_config(), logger=IMAGE_LOGGER)
    np.testing.assert_array_equal(result['MOON_A']['masks']['body_latitude'], mask)
...
def test_simulated_body_value_is_constant_in_range() -> None:
    ...
    snap, mask = _sim_snapshot_with_moon_a()
    result = create_body_backplanes(snap, _bodies_config().as_config(), logger=IMAGE_LOGGER)
    values = np.unique(result['MOON_A']['arrays']['body_latitude'][mask])
    assert len(values) == 1
...
def test_simulated_body_value_deterministic_within_process() -> None:
    ...
    val_first = first['MOON_A']['arrays']['body_latitude'][2, 3]
    val_second = second['MOON_A']['arrays']['body_latitude'][2, 3]
...
full, _ = _create_simulated_body_backplane(snapshot, 'MOON_A', 'body_latitude', 2, 3, 3, 5)
...
def test_simulated_body_value_deterministic_across_processes() -> None:
    ...
    in this process for the same MOON_A / body_latitude inputs.
    """
    snap, _ = _sim_snapshot_with_moon_a()
    result = create_body_backplanes(snap, _bodies_config().as_config(), logger=IMAGE_LOGGER)
    in_process = float(result['MOON_A']['arrays']['body_latitude'][2, 3])
    values = []
    for seed in ('1', '2', '3'):
        env = dict(os.environ)
...
def test_body_outside_fov_contributes_nothing() -> None:
    ...
    mask = np.ones(SHAPE_VU, dtype=bool)
    inventory = {
        'MOON_A': inventory_entry(u_min=-20, u_max=-10, v_min=2, v_max=3, body_range=1.0e5)
    }
    snap = make_snapshot(
        shape_vu=SHAPE_VU,
        simulated=True,
...
def test_body_bounding_box_clipped_into_sensor() -> None:
    ...
    mask = np.ones(SHAPE_VU, dtype=bool)
    inventory = {'MOON_A': inventory_entry(u_min=-2, u_max=4, v_min=3, v_max=9, body_range=1.0e5)}
    snap = make_snapshot(
        shape_vu=SHAPE_VU,
        simulated=True,
        sim_inventory=inventory,
...
def test_inventory_queried_for_planet_and_satellites() -> None:
    ...
    config = FakeBackplanesConfig(bodies=[LAT_CFG], satellites={'PLANET': ['MOON_A', 'MOON_B']})
    create_body_backplanes(snap, config.as_config(), logger=IMAGE_LOGGER)
    assert snap.inventory_calls == [['PLANET', 'MOON_A', 'MOON_B']]


def _real_snapshot_with_moon_a() -> HermeticObs:
    """Build a non-simulated snapshot with a canned MOON_A inventory (u 3..6, v 2..4)."""
    inventory = {'MOON_A': inventory_entry(u_min=3, u_max=6, v_min=2, v_max=4, body_range=200000.0)}
...
def test_real_body_masked_pixels_carry_the_masked_value(
    ...
    result = create_body_backplanes(snap, _bodies_config().as_config(), logger=IMAGE_LOGGER)
    arr = result['MOON_A']['arrays']['body_latitude']
    mask = result['MOON_A']['masks']['body_latitude']
...
def test_simulated_backplane_mask_map_lookup_is_case_insensitive() -> None:
    ...
    mask = np.zeros(SHAPE_VU, dtype=bool)
    mask[2:4, 3:6] = True
    snap = make_snapshot(shape_vu=SHAPE_VU, simulated=True, sim_body_mask_map={'MOON_A': mask})
    full, full_mask = _create_simulated_body_backplane(snap, 'moon_a', 'body_latitude', 2, 3, 3, 5)
