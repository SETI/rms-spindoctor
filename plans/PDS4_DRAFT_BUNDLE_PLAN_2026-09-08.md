...
the primary, an `Array_2D` with `offset` `datLoc`, axis sizes from
`NAXIS1`/`NAXIS2`, `Element_Array` `data_type` from `BITPIX`
(`IEEE754MSBSingle` for -32, `SignedMSB4` for 32), and `unit` from `BUNIT`.
...
3. **The cohort builder**, as `tests/mini_nav_results/cohort.py`: it
   ...
   the tests take as a fixture. `__main__.py` becomes the two-argument
   entry point of section 3.12 -- set name, then output directory, both
   required for both sets. The statistics regeneration command changes shape
   with it, so update the one place it is written down (the package
   docstring) and any test docstring that quotes it.
