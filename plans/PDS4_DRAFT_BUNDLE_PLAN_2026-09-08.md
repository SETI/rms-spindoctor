...
Three parts of it are conditional: `document/user_guide/` is written only
...
holds gives that table a row (Phase 7), and `miscellaneous/` itself only when
one of its index tables has a row. Both guides and the built trees say so.
