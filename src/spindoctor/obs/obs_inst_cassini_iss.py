...
"""The label keywords the host publishes, in the label's own order.

These are the observation keywords of a Cassini ISS image label, which the image's
own VICAR label states in its property blocks.  The list is written out rather than
taken from the label so that a label carrying a keyword it omits is a difference a
reader can see, rather than one that drops a keyword in silence.
"""


def _label_metadata(label: Mapping[str, Any]) -> dict[str, Any]:
    """Return the metadata an image's label states, under the label's own keyword names.

    Each value is the label's own, as the label states it: a number stays a number, text
    stays text, a time is the label's own spelling of it, and a sequence stays a sequence.
    A keyword the label lacks is published as None.

    Parameters:
        label: The image's VICAR label items.

    Returns:
        The value of each keyword ``_LABEL_METADATA`` names, keyed by that keyword, in
        its order.
    """
    return {keyword: label.get(keyword, None) for keyword in _LABEL_METADATA}
...
class ObsCassiniISS(ObsSnapshotInst):
    ...
    def get_public_metadata(self) -> dict[str, Any]:
        ...
        After these come the metadata the image's label states, each under the label's
        own keyword name and with the value the label states: a number stays a number,
        text stays text, a time is the label's own spelling, and a sequence stays a
        sequence.  A keyword the label lacks is published as None.  ``_LABEL_METADATA``
        lists the keywords.
