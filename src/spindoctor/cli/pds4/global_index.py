...
def _write_index(
    ...
    *,
    ...
    least one (``records`` has a minimum of 1 in ``PDS4_PDS_1O00.xsd``), so a table
    with no row cannot be described: neither it nor its label is written.  That is no
    failure, since a bundle can hold no image with ring backplanes, and the log says so
    at info level.
...
def generate_global_index_files(
    ...
    hold no image with ring backplanes; the log says so at info level.  With neither
    table written, though, the miscellaneous collection holds nothing of its own, and
    that counts (below).
