...
frame labelled ``Stars`` was fixed against a catalog and is worth believing to
...
# the field centre, and the disagreement is a few parts in a thousand -- enough
# to matter to an absolute scale, not to the pixel differences measured here.
NAC_PLATE_SCALE_URAD = 5.9946

DEFAULT_BUNDLE_DIR = Path('/data/fring-bundles/pds4/data_reproj_img')
...
def suppl_path(
    ...
    *,
    ...
    return bundle_dir / observation_id.lower() / f'{bare[1:]}{bare[0].lower()}_reproj_img_suppl.txt'


def read_bundle_pointing(path: Path) -> BundlePointing | None:
    ...
    if matrix:
        row = np.array([float(matrix.group(i)) for i in (7, 8, 9)], dtype=float)
    ...
    if not (ra and dec):
        return None
    return BundlePointing(
        vector_from_ra_dec(float(ra.group(1)), float(dec.group(1))), navigation_type, False
    )


def bundle_image_names(*, observation_id: str, bundle_dir: Path = DEFAULT_BUNDLE_DIR) -> list[str]:
    ...
    """
    directory = bundle_dir / observation_id.lower()
