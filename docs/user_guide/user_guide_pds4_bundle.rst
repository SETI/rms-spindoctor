...
* ``sd_create_bundle labels`` exits 1 without writing anything if the bundle
  ...
  the observation, its backplanes were generated by an earlier version, which
  recorded ring backplanes without their ring target and the incidence angle of
  sunlight on the rings, or its backplanes cover no target at all, no body and no
  rings, since a data label has to name one. For such a statistic, or backplanes
