...
* The dataset's template directory (see `Templates`_), from which it copies the readme,
  ...
  metakernel and, when the directory holds it, the user guide
...
* ``sd_create_bundle summary`` exits 1 without writing anything if a file it needs
  from the template directory is missing (the user guide apart), or if the bundle has
...
* The template for the user guide's label and, when it exists, the user guide itself:
  for ``coiss_saturn``, ``cassini-iss-saturn-backplanes-user-guide.lblx`` and
  ``cassini-iss-saturn-backplanes-user-guide.pdf``. The user guide is the one file the
  summary pass can do without, and ``cassini_iss_saturn_1.0`` does not hold it.
...
* **Template not found**: the pass exits before writing anything, naming each
  ...
  every file listed above, the user guide apart.

* **User guide not in the template directory**: the summary pass warns, naming the
  file, and writes the bundle without a user guide. Put the PDF in the template
